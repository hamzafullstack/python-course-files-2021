...
lab_frame = ttk.LabelFrame(win, text='enter some thing : ')
lab_frame.grid(row=0, column=0)
...
for i in range(len(labels)): # loop on labels with len function
    ...
    current_label = ttk.Label(lab_frame, text=labels[i])
    current_label.grid(row=i, column=0, sticky=tk.W, padx=3, pady=3)
...
for i in user_data:
    ...
    current_entrybox = ttk.Entry(lab_frame, width=16, textvariable=user_data[i])
